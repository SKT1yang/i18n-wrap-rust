 ...
 * @path: \permission\src\src\views\Route\RouteModifyModal.vue
-->
<template>
  <Modal :title="title" v-model:open="dialogVisible" :width="1200">
    <Form :model="dataForm" :rules="rules" ref="formRef" :label-col="{ span: 6 }">
      <FormItem name="pid" label="上级路由" :label-col="{ span: 3 }">
        <TreeSelect v-model:value="dataForm.pid" checkStrictly show-search :tree-data="routeTree"
      ...
      <Tabs v-model:active-key="activeName">
        ...
        <TabPane tab="深度配置" key="advance">
          ...
          <div class="grid grid-cols-2">
            ...
            <FormItem name="hideChildrenInMenu" label="隐藏子页面路由:" tooltip="子路由正常注册，但在菜单上不显示，只显示当前菜单，当所有子菜单都不希望在菜单显示时很好用">
...
import { ref } from 'vue';
import { Modal, Form, FormItem, Input, InputNumber, Button, TreeSelect, Switch, Tabs, TabPane } from 'ant-design-vue';
import { useRouteModify } from '../../controller/useRoute'
...
const props = defineProps({
  ...
  current: {
    type: Object as PropType<PermissionTreeItem>,
    default: () => { }
  },
  systemId: {
    type: Number,
...
const { dataForm, rules, dialogVisible, title, activeName, routeTree, handleSubmit, closeModal } = useRouteModify(props, emit, formRef)
