...
-->
<template>
  <div class="gradient border-box w-full h-14px relative cursor-pointer rounded-10px mb-8px" :style="pointsStyle"
    @click="handleAddPoit">
    <div ref="pointsContainerRef" class="wh-full relative">
      <GradientPoint v-for="(point, index) in colorPickerState.points" :key="point.id" :index="index" :point="point"
        :positions="pointsContainerBoxInfo" :width="pointsContainerBoxInfo?.width" />
    </div>
  </div>
</template>
...
} from '@/domains/ColorPicker/src/helpers/index'
import { v4 as uuidv4 } from 'uuid'
import { hsvToRgb } from '@/domains/ColorPicker/src/helpers'
import type { IColorState, IPoitItem } from '@/domains/ColorPicker/src/types'
