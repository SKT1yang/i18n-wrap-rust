...
<template>
  <Tabs v-model:active-key="activeKey" centered destroy-inactive-tab-pane @change="handleChange">
    ...
    <TabPane key="application" :tab="t('应用软件')" v-if="hasLicense">
...
import PortInfoHost from '@/domains/port/views/PortInfoHost.vue';
import ApplicationInformation from '@/domains/app/views/ApplicationInformation.vue';
...
import RunStatusHost from '@/domains/hardware/views/host/RunStatusHost.vue';
/* api */
...
const { activeKey, handleChange } = useTabsSwitch(["overview", "info", 'internetworking', "log", "flow", "event", "vul", "relation", "application", "Database"])
