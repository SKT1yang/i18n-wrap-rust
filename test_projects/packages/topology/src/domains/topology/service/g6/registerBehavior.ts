...
import { isFunction } from '@guolisec/utils';
import { useTopologyStoreWithOut } from '../../model/store';
import { EDGE_TYPE, BEHAVIOR_TYPE } from '../../types/enum';
...
const registerBehavior = (G6) => {
  const topologyStore = useTopologyStoreWithOut();
  // 设备类型节点
  G6.registerBehavior(BEHAVIOR_TYPE.CREATE_DYNAMIC_EDGE, {
      ...
      if (self.addingEdge && self.edge) {
        ...
        self.edge = graph.addItem(
          'edge',
          {
            ...
            ...edgeConfig,
            ...{
              type: topologyStore.edgeName || EDGE_TYPE.ASSET_POLYLINE_EDGE,
            },
          },
          false
        );
    ...
    cancelCreating(ev: IG6GraphEvent) {
      ...
      if (
        self.addingEdge &&
        (self.edge === currentEdge || ev.target?.isCanvas?.())
      ) {
        if (self.edge && !self.edge.destroyed)
