<template>
  <div>
    ...
    <tabs v-model:activekey="activeKey" type="card" @change="handleTabChange">
      <tabpane :key="1006" :tab="t('全部')"></tabpane>
...
const activeKey = ref(1006);
async function getEventTypeLevel() {
	eventTypeList.value = await getIcsTabApi();
	eventTypeIn.value = [];
...
onMounted(() => {
	...
	getLogSourceTypeNameApi({ eventType: 1006 }).then((res) => {
	...
	getIcsLogSourceTypeName().then((res) => {
