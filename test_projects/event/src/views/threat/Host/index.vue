<template>
  <div>
    ...
    <tabs v-model:activekey="activeKey" type="card" @change="handleTabChange">
      <tabpane :key="1202" :tab="t('全部')"></tabpane>
...
const activeKey = ref(1202);
async function getEventTypeLevel() {
	eventTypeList.value = await getEventTypeLevelApi(1202);
	eventTypeIn.value = [];
...
onMounted(() => {
	...
	getEventNameApi({ eventType: 1202 }).then((res) => {
	...
	getLogSourceTypeNameApi({ eventType: 1202 }).then((res) => {
	...
	getLogSourceNameApi({ eventType: 1202 }).then((res) => {
